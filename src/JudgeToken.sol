    ...
    ERC20Capped
{
    ...
    {
        if (initialSupply > 100_000 * 10 ** decimals())
            revert InitialMintExceedsLimit();
    ...
    function decreaseMintableUnallocatedJudge(uint256 amount) internal {
        if (amount > mintableUnallocatedJudge)
            revert AmountExceedsMintableUnallocatedJudge();
        unchecked {
    ...
    function decreaseMintableAllocatedJudge(uint256 amount) internal {
        if (amount > mintableAllocatedJudge)
            revert AmountExceedsMintableAllocatedJudge();
        unchecked {
